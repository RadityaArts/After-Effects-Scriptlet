/**
 * @name Fit_to_Comp_Height
 * @version 1.0
 * ================================
 * Scales selected layers to fit the composition height while maintaining aspect ratio
...
(function fitToCompHeight() {
    ...
    /**
     * Calculate adjusted height accounting for rotation
     */
    function getAdjustedHeight(bounds, rotation) {
        var angleInRadians = rotation * (Math.PI / 180);
        return Math.abs(bounds.width * Math.sin(angleInRadians)) +
               Math.abs(bounds.height * Math.cos(angleInRadians));
    }
    ...
    try {
        ...
        for (var i = 0; i < selectedLayers.length; i++) {
            ...
            var adjustedHeight = getAdjustedHeight(bounds, rotation);

            // Calculate and apply scale to fit comp height
